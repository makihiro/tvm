...
To run this notebook, you need to install tvm.
...
# The Relay CI does not enable OpenGL yet. But the user can run this script.
...
#   Relay computation graph.
#
# - A function that downloads a file that contains the name of all the image
#   classes in this model.
#
def load_mxnet_resnet():
    """Load a pretrained resnet model from MXNet and transform that into Relay
       format.

    Returns
    -------
    func : tvm.relay.Function
        The loaded resnet Relay function.
    ...
    # Transform the mxnet model into Relay.
    # We want a probability so add a softmax operator.
    data_shape = (1, 3, 224, 224)
    func, params = relay.frontend.from_mxnet(block, {'data': data_shape})
    func = relay.Function(func.params, relay.nn.softmax(func.body), None, func.type_params, func.attrs)

    print("- Model loaded!")
    return func, params, data_shape, (1, 1000)
...
# Compile the Model
# -----------------
# Here we define a function that invokes the Relay compiler.
#
def compile_net(func, target_host, target, data_shape, params):
    ...
    with relay.build_config(opt_level=0):
        deploy_graph, lib, deploy_params = relay.build(
            func,
...
# ----------------------
# In this demo, we will compile the model targeting the local machine.
#
...
def deploy_local():
    ...
    # Load resnet model.
    func, params, data_shape, out_shape = load_mxnet_resnet()
    ...
    # Note we need to save all three files returned from the Relay compiler.
